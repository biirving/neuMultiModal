...
path_source_files = "neuralcatcher/hateful_memes"

dataset = load_dataset(path_source_files)
data = pd.DataFrame(dataset['train'])
print("count", data['label'].value_counts())


image_feature_extractor = 'google/vit-base-patch16-224-in21k'
image_model = 'google/vit-base-patch16-224'

text_feature_extractor = 'facebook/bart-large'
...
# Image Models
feature_extractor_img = ViTFeatureExtractor.from_pretrained(image_feature_extractor)
model_img = ViTModel.from_pretrained(image_model)

# Text Models
tokenizer_txt = BartTokenizer.from_pretrained(text_feature_extractor)
...
def extract_fused_embeddings(each, data):
    ...
    fuse = final_txt_embeddings.cuda()
...
head = MLP(in_channels=1024,num_classes=1).cuda()
...
pd.DataFrame(pred).to_csv("Pred_txt.csv")
pd.DataFrame(acc).to_csv("Acc_txt.csv")
